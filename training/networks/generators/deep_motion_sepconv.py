...
def get_network_v1_prelu(x, training):

    x_shape = tf.shape(x)

    def block(tensor, filters):
        down_conv1 = tf.layers.conv2d(tensor, filters, 3, padding='same')
        down_conv1_prelu = _tf.prelu(down_conv1, 'prelu1')
        down_conv2 = tf.layers.conv2d(down_conv1_prelu, filters, 3, padding='same')
        down_conv2_prelu = _tf.prelu(down_conv2, 'prelu2')
        down_conv3 = tf.layers.conv2d(down_conv2_prelu, filters, 3, padding='same')
        return _tf.prelu(down_conv3, 'prelu3')

    def upsample(tensor, filters, target):
        up_shape = tf.shape(target)
        up = tf.image.resize_bilinear(tensor, [up_shape[1], up_shape[2]])
        up_conv = tf.layers.conv2d(up, filters, 3, padding='same')
        return _tf.prelu(up_conv, 'prelu_up')

    def subnet(tensor):
        sub_conv1 = tf.layers.conv2d(tensor, 64, 3, padding='same')
        sub_conv1_prelu = _tf.prelu(sub_conv1, 'prelu1')
        sub_conv2 = tf.layers.conv2d(sub_conv1_prelu, 64, 3, padding='same')
        sub_conv2_prelu = _tf.prelu(sub_conv2, 'prelu2')
        sub_conv3 = tf.layers.conv2d(sub_conv2_prelu, 51, 3, padding='same')
        sub_conv3_prelu = _tf.prelu(sub_conv3, 'prelu3')
        sub_up = tf.image.resize_bilinear(sub_conv3_prelu, [x_shape[1], x_shape[2]])
        return tf.layers.conv2d(sub_up, 51, 3, padding='same')
    ...
    with tf.variable_scope('unet', None, [x]):
        ...
        with tf.variable_scope('sepconv', None, [up2, x]):
            ...
            with tf.variable_scope('frame_0', None, [up2, x]):
                with tf.variable_scope('subnet_1', None, [up2]):
                    kv_0 = subnet(up2)
                with tf.variable_scope('subnet_2', None, [up2]):
                    kh_0 = subnet(up2)
                frame_0 = SEPCONV_MODULE.sepconv(x[:, :, :, :3], kv_0, kh_0)

            with tf.variable_scope('frame_1', None, [up2, x]):
                with tf.variable_scope('subnet_3', None, [up2]):
                    kv_1 = subnet(up2)
                with tf.variable_scope('subnet_4', None, [up2]):
                    kh_1 = subnet(up2)
                frame_1 = SEPCONV_MODULE.sepconv(x[:, :, :, 3:], kv_1, kh_1)
