...
def get_network_v1(x, training):
    ...
    unet = get_base(x)
    return tf.layers.conv2d(unet, 3, 1, activation=tf.nn.leaky_relu)
...
def get_network_v1_with_flow_v1(x, training):
    ...
    with tf.variable_scope('rgb_unet', None, [x_stack]): 
        ...
        return tf.layers.conv2d(unet_rgb, 3, 1, activation=tf.nn.leaky_relu)
...
def get_network_v2(x):
    ...
    with tf.variable_scope('unet', None, [x]):
        ...
        with tf.variable_scope('decoder', None, [h_norm2, norm4, norm3, norm2, norm1]):
            ...
            return tf.layers.separable_conv2d(up1, 3, 3, activation=tf.nn.leaky_relu, padding='same')
...
def get_network_v2_1(x):
    ...
    with tf.variable_scope('unet', None, [x]):
        ...
        with tf.variable_scope('decoder', None, [h_norm2, norm4, norm3, norm2, norm1]):
            ...
            return tf.layers.conv2d(up1, 3, 3, activation=tf.nn.leaky_relu, padding='same')
